    ...
    <div className="flex flex-col gap-5 h-full">
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
          <FiTool className="w-4 h-4 text-cyan-400" />
        ...
        <div className="flex flex-wrap gap-1">
          {INSTALL_METHODS.map((m) => (
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-medium transition-all ${activeTab === m.id
                ? 'bg-violet-500/10 text-violet-400 border border-violet-500/20'
                : 'text-zinc-500 border border-white/5 hover:text-zinc-400 hover:border-white/10'
                }`}
            >
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 border-b border-white/5">
            <div>
              ...
              <p className="text-[10px] text-zinc-500 mt-0.5">{activeMethod.desc}</p>
            </div>
        ...
        <div className="space-y-1">
          {FAQ_ITEMS.map((faq) => (
            ...
            >
              ...
              {expandedFaq === faq.id && (
                <div className="px-3 pb-2 text-[11px] text-zinc-500 leading-relaxed border-t border-white/5 pt-2">
                  {faq.a}
                </div>
              )}
            </div>
  ...
  return (
    <button
      ...
      className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold
        text-zinc-500 hover:text-violet-400 border border-white/10 hover:border-violet-400/20 transition-all"
    >
      {copied ? (
        <>
